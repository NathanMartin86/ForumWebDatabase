...
public class Main {
    ...
    public static Message selectMessage (Connection connection, int id) throws SQLException {
        Message message = null;
        PreparedStatement stmt = connection.prepareStatement("SELECT * FROM messages INNER JOIN users ON messages.user_id = users.id WHERE messages.id = ?");
        stmt.setInt(1, id);
        ResultSet results = stmt.executeQuery();
        if (results.next()){
            message = new Message();
